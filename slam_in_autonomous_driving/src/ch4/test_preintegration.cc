...
TEST(PREINTEGRATION_TEST, ESKF_TEST) {
    ...
    io.SetIMUProcessFunc([&](const sad::IMU& imu) {
          /// 1. Add IMU if it's not initialized
          ...
          // Rico: eskf predict is just R = R1 * exp((w-bg)*delta_t)
          // See eskf.hpp, line 229.
          eskf.Predict(imu);

          if (preinteg) {
              // Integrate dR, dp, dv. Update jacobians: dp_dba, dp_dbg, dv_dba, dv_dbg. 
              // See imu_preintegration.cc, line 14
              ...
              if (last_state_set) {
                  ...
                  //TODO
                  std::cout<<"r diff: "<<(pred_of_preinteg.R_.inverse() * pred_of_eskf.R_).log().norm()<<std::endl;
              }
          }
      })
        .SetGNSSProcessFunc([&](const sad::GNSS& gnss) {
            ...
            if (last_state_set && last_gnss_set) {
                ...
                // TODO: 
                //TODO
                std::cout<<"Rico to recover optimization!!"<<std::endl;
            ...
            // 重置预积分, so it can be used for the next one?
